...
df <- readxl::read_xlsx("raw_data/Looks vs Personality.xlsx") %>%
  ...
  mutate(country_name = countrycode::countrycode(country, "country.name", "country.name")) %>%
...
ggplot(data = world) + 
  aes(x = Question_short_wrap, y = Percent) +
  aes(fill = `Rank (text)`) +
  facet_grid(Gender ~ .) +
  geom_col() +
  coord_flip() +
  scale_fill_manual(values = colorRampPalette(RColorBrewer::brewer.pal(9, "Purples"))(6)[1:6], guide = guide_legend(reverse = TRUE)) +
  labs(fill = "") +
  xlab("") +
...
`r paste(knitr::knit(text = partial_knit_chunks("traits")), collapse = "\n")`
